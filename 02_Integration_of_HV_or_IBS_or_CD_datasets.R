...
# ALL
IBS_list = c("GC1003333_C5","GC1003334_D5","GC1004260_D1",...)   

sample_list<-list()
for (i in (1:length(IBS_list))) {
  sample=IBS_list[i]
  if (file.exists(paste0("/path/Result_",sample,"/",sample,"_FC.rds"))){
    so<-readRDS(paste0("/path/Result_",sample,"/",sample,"_FC.rds"))
          sample_list[[i]]<-so
}
}  
head(sample_list[[i]])
...
Seurat.markers <- FindAllMarkers(Seurat_reso,  min.pct = 0.1, logfc.threshold = 0.25, max.cells.per.ident = Inf, latent.vars = NULL,min.cells.feature = 0, min.cells.group = 0)
write.table(Seurat.markers, paste0("Combined_res.",resolution,"_markers_unannotated_Clusters.txt"),col.names=NA, sep="\t")
